...
A design editor with advanced export options including format selection (JPEG, PNG, PDF), quality settings, resolution control, and page range selection for multi-page documents. Built with [CE.SDK](https://img.ly/creative-sdk) by [IMG.LY](https://img.ly), runs entirely in the browser with no server dependencies.

<p>
  <a href="https://img.ly/docs/cesdk/starterkits/export-options-editor/">Documentation</a> |
...
```bash
git clone https://github.com/imgly/starterkit-export-options-editor-ts-web.git
cd starterkit-export-options-editor-ts-web
```
...
│       ├── config/           # Design editor configuration
│       └── plugins/
│           └── export-design-panel.ts   # Custom export panel
├── index.html                # Editor container
├── package.json
└── vite.config.ts
```
